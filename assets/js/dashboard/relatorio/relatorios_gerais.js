...
function enviar(relatorio, filtro) {
	window.open(
		base_url +
			"/relatorio/gera_relatorio_geral/" +
			relatorio +
			"/" +
			filtro +
			"/" +
...
$("#gerar_relatorio").click(function() {
	...
	$.post(base_url + "/Relatorio/select_os_by_filter", form).done(function(
		response
	) {
		...
		if (response.code == 200) {
			...
			if (response.data > 0){
				$("#p_qtd").text(
					"Esse relatório terá " + response.data + " ordens de serviço!"
				);
				$("#confirmar").prop("disabled", false);
			}
			else {
				$("#p_qtd").text(
					"Não é permitido criar um relatório com 0 ordens de serviço."
				);
...
$("#confirmar").click(function() {
	...
	$.post(base_url + "/Relatorio/create_new_report", form).done(function(
		response
	) {
		...
		if (response.code == 200) {
			...
			window.location.href =
				base_url + "/Relatorio/detalhes/" + response.data.id;
		} else if (response.code == 400) {
			alerts("failed", "Erro!", response.data.mensagem);
		} else {
			alerts(
				"failed",
				"Erro!",
				"Tem de conexão excedido.<br>Por favor, recarregue a página."
			);
		}
	},
	"json");
